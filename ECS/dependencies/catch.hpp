...
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, ...)                                                               \
    ...
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                                                         \
        &INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____)::test, #ClassName, Catch::NameAndDesc(__VA_ARGS__), \
        CATCH_INTERNAL_LINEINFO);                                                                                     \
    }                                                                                                                 \
...
class TestCaseTracker {
   public:
    ...
    bool enterSection(std::string const& name) {
        ...
        if (m_completedASectionThisRun || child->runState() == TrackedSection::Completed) return false;
...
struct FatalConditionHandler {
    ...
    void reset() {
        if (m_isSet) {
            for (std::size_t i = 0; i < sizeof(signalDefs) / sizeof(SignalDefs); ++i) signal(signalDefs[i].id, SIG_DFL);
            m_isSet = false;
        }
    }
...
inline bool isReservedTag(std::string const& tag) { return TestCaseInfo::None && tag.size() > 0 && !isalnum(tag[0]); }
...
Section::~Section() {
    if (m_sectionIncluded) getResultCapture().sectionEnded(m_info, m_assertions, m_timer.getElapsedSeconds());
}
...
struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
    ...
    virtual void testCaseEnded(TestCaseStats const& /* _testCaseStats */) { currentTestCaseInfo.reset(); }
...
class XmlReporter : public StreamingReporterBase {
   ...
   public:  // StreamingReporterBase
    ...
    virtual void noMatchingTestCases(std::string const& s) { StreamingReporterBase::noMatchingTestCases(s); }
...
struct ConsoleReporter : StreamingReporterBase {
   ...
   private:
    ...
    void lazyPrintRunInfo() {
        ...
        if (libraryVersion.branchName != std::string("master")) stream << " (" << libraryVersion.branchName << ")";
...
#define CATCH_CHECK_THROWS_AS(expr, exceptionType)                                                                   \
    INTERNAL_CATCH_THROWS_AS(expr, exceptionType, Catch::ResultDisposition::ContinueOnFailure, "CATCH_CHECK_THROWS_" \
                                                                                               "AS")
